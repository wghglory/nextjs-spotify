...
## Create spotify.ts helper in `lib`

Describe the scope, shown in the auth login popup. Register spotify webApi.
...
This file registers provider, and handle tokens.
...
## Update `pages/_app.tsx` with `SessionProvider`
...
Configure redirect uri in spotify app settings:

![spotify redirect uri setting](redirect_uri.png)
...
## Get user info in any page by `useSession`
