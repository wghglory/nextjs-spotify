      ...
      spotifyApi
        ...
        .then((res) => {
          ...
          if (res.body.items?.[0]?.id) {
            setPlaylistId(res.body.items[0].id); // set default playlistId
          }
        })
        .catch((err) => {
          console.warn(err);
  ...
  return (
    <div className="text-gray-500 p-5 text-xs lg:text-sm border-r border-gray-900 overflow-y-auto no-scrollbar h-screen hidden md:inline-flex sm:max-w-[12rem] lg:max-w-[15rem]">
      <div className="space-y-4">
        <div role="banner">
          <Link href="/">
            <a className="hover:text-white flex">
              <svg viewBox="0 0 1134 340" className="w-36">
                <title>Spotify</title>
                <path
                  fill="currentColor"
        ...
        {playlist.map((p) => (
          <p key={p.id} onClick={() => setPlaylistId(p.id)} className="cursor-pointer hover:text-white">
            {p.name}
          </p>
        ))}
      </div>
